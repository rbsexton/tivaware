         ...
         uint32_t ui32NetMask, uint32_t ui32GWAddr, uint32_t ui32IPMode)
{
  ...
  // Apply errata, ETH#02 Ethernet May not Initialize After System Reset

  uint32_t mask = ~(FLASH_CONF_FPFOFF | FLASH_CONF_FPFON) & HWREG(FLASH_BASE + FLASH_CONF);
  HWREG(FLASH_BASE + FLASH_CONF) = mask | FLASH_CONF_FPFOFF;
    ...
    // End of Errata
    HWREG(FLASH_BASE + FLASH_CONF) = mask | FLASH_CONF_FPFON;
